...
class PHPExcel_Worksheet_AutoFilter
{
    ...
    public function setRange($pRange = '')
    {
        ...
        if (strpos($pRange, ':') !== false) {
            ...
            throw new PHPExcel_Exception(
                'Autofilter must be set on a range of cells.'
            );
        }
        ...
        if (empty($pRange)) {
            ...
            list($rangeStart, $rangeEnd) = PHPExcel_Cell::rangeBoundaries(
                $this->range
            );
    ...
    public function getColumn($pColumn)
    {
        ...
        if (!isset($this->columns[$pColumn])) {
            $this->columns[$pColumn] = new PHPExcel_Worksheet_AutoFilter_Column(
                $pColumn,
                $this
            );
        }
    ...
    public function getColumnByOffset($pColumnOffset = 0)
    {
        ...
        $pColumn = PHPExcel_Cell::stringFromColumnIndex(
            $rangeStart[0] + $pColumnOffset - 1
        );
    ...
    public function setColumn($pColumn)
    {
        if (is_string($pColumn) && !empty($pColumn)) {
            ...
            is_object($pColumn) &&
            $pColumn instanceof PHPExcel_Worksheet_AutoFilter_Column
        ) {
            $column = $pColumn->getColumnIndex();
        } else {
            throw new PHPExcel_Exception(
                'Column is not within the autofilter range.'
            );
        }
    ...
    private static function filterTestInCustomDataSet($cellValue, $ruleSet)
    {
        ...
        $join = $ruleSet['join'];
        $customRuleForBlanks = isset($ruleSet['customRuleForBlanks'])
            ? $ruleSet['customRuleForBlanks']
            : false;
        ...
        $returnVal =
            $join ==
            PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_COLUMN_JOIN_AND;
    ...
    private static function filterTestInPeriodDateSet($cellValue, $monthSet)
    {
        ...
        if (is_numeric($cellValue)) {
            $dateValue = date(
                'm',
                PHPExcel_Shared_Date::ExcelToPHP($cellValue)
            );
    ...
    private function dynamicFilterDateRange($dynamicRuleType, &$filterColumn)
    {
        ...
        switch ($dynamicRuleType) {
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_TOMORROW:
                $maxVal = (int) PHPExcel_Shared_Date::PHPtoExcel(
                    strtotime('+1 day', $baseDate)
                );
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_YEARTODATE:
                ...
                $val = (int) PHPExcel_Shared_Date::PHPToExcel(
                    gmmktime(0, 0, 0, 1, 1, date('Y', $baseDate))
                );
                break;
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTYEAR:
                $maxVal = (int) PHPExcel_Shared_Date::PHPToExcel(
                    gmmktime(0, 0, 0, 31, 12, date('Y', $baseDate))
                );
                ++$maxVal;
                $val = (int) PHPExcel_Shared_Date::PHPToExcel(
                    gmmktime(0, 0, 0, 1, 1, date('Y', $baseDate))
            ...
            case PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_NEXTWEEK:
                ...
                $val =
                    (int) PHPExcel_Shared_Date::PHPToExcel($baseDate) -
                    $dayOfWeek;
                $maxVal = $val + 7;
                break;
        }
    ...
    private function calculateTopTenValue(
        $columnID,
        $startRow,
        $endRow,
        $ruleType,
        ...
        $dataValues = PHPExcel_Calculation_Functions::flattenArray(
            $this->workSheet->rangeToArray($range, null, true, false)
        );
            ...
            switch ($filterColumn->getFilterType()) {
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_FILTER:
                    ...
                    if (
                        ...
                        $arguments['dateTime'] = array_filter(
                            $arguments['dateTime']
                        );
                        $columnFilterTests[$columnID] = [
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_DYNAMICFILTER:
                    ...
                    foreach ($rules as $rule) {
                        ...
                        if (
                            $dynamicRuleType ==
                                PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_ABOVEAVERAGE ||
                            $dynamicRuleType ==
                                PHPExcel_Worksheet_AutoFilter_Column_Rule::AUTOFILTER_RULETYPE_DYNAMIC_BELOWAVERAGE
                        ) {
                            ...
                            $averageFormula =
                                ...
                                $rangeEnd[1] .
                                ')';
                            $average = PHPExcel_Calculation::getInstance()->calculateFormula(
                                $averageFormula,
                                null,
                ...
                case PHPExcel_Worksheet_AutoFilter_Column::AUTOFILTER_FILTERTYPE_TOPTENFILTER:
                    ...
                    $maxVal = $this->calculateTopTenValue(
                        $columnID,
                        $rangeStart[1] + 1,
                        $rangeEnd[1],
                        $toptenRuleType,

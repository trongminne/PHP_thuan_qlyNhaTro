...
class PHPExcel_Calculation_Functions
{
    ...
    {
        $condition = PHPExcel_Calculation_Functions::flattenSingleValue(
            $condition
        );
        ...
        if (!in_array($condition[0], ['>', '<', '='])) {
            if (!is_numeric($condition)) {
                $condition = PHPExcel_Calculation::wrapResult(
            ...
            if (!is_numeric($operand)) {
                ...
                $operand = PHPExcel_Calculation::wrapResult(
                    strtoupper($operand)
                );
            }
...
if (
    ...
    function mb_str_replace($search, $replace, $subject)
    {
        ...
        foreach ((array) $search as $key => $s) {
            ...
            while ($pos !== false) {
                $subject =
                    mb_substr($subject, 0, $pos, 'UTF-8') .
                    $r .
                    mb_substr(
                        $subject,
                        $pos + mb_strlen($s, 'UTF-8'),
                        65535,
                        'UTF-8'
                    );
                $pos = mb_strpos(

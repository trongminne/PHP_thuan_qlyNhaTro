...
class PHPExcel_Calculation
{
            ...
            if ($locale != 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    if (strpos($localeFunction, '=') !== false) {
                        ...
                        $lfName = trim($lfName);
                        if (
                            isset(self::$PHPExcelFunctions[$fName]) &&
                            $lfName != '' &&
                            $fName != $lfName
                        ) {
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        ...
                        if (strpos($localeSetting, '=') !== false) {
                            list($settingName, $settingValue) = explode(
                                '=',
                                $localeSetting
                            );
                            ...
                            switch ($settingName) {
                                case 'ARGUMENTSEPARATOR':
                                    self::$localeArgumentSeparator = trim(
                                        $settingValue
                                    );
                                    break;
                            }
    ...
    private static function translateFormula(
        $from,
        $to,
        $formula,
        $fromSeparator,
        ...
        if (self::$localeLanguage !== 'en_us') {
            ...
            if (strpos($formula, '"') !== false) {
                ...
                $formula = self::translateSeparator(
                    $fromSeparator,
                    $toSeparator,
                    $formula,
                    $inBraces
    ...
    public function _translateFormulaToLocale($formula)
    {
        if (self::$functionReplaceFromExcel === null) {
            ...
            foreach (array_keys(self::$localeFunctions) as $excelFunctionName) {
                self::$functionReplaceFromExcel[] =
                    '/(@?[^\w\.])' .
                    preg_quote($excelFunctionName) .
                    '([\s]*\()/Ui';
            }
            foreach (array_keys(self::$localeBoolean) as $excelBoolean) {
                self::$functionReplaceFromExcel[] =
                    '/(@?[^\w\.])' . preg_quote($excelBoolean) . '([^\w\.])/Ui';
            }
        }
        ...
        if (self::$functionReplaceToLocale === null) {
            ...
            foreach (
                array_values(self::$localeFunctions)
                as $localeFunctionName
            ) {
                self::$functionReplaceToLocale[] =
                    '$1' . trim($localeFunctionName) . '$2';
            }
            foreach (array_values(self::$localeBoolean) as $localeBoolean) {
                self::$functionReplaceToLocale[] =
                    '$1' . trim($localeBoolean) . '$2';
            }
        }
    ...
    public function _translateFormulaToEnglish($formula)
    {
        ...
        if (self::$functionReplaceToExcel === null) {
            ...
            foreach (array_keys(self::$localeFunctions) as $excelFunctionName) {
                self::$functionReplaceToExcel[] =
                    '$1' . trim($excelFunctionName) . '$2';
            }
            foreach (array_keys(self::$localeBoolean) as $excelBoolean) {
                self::$functionReplaceToExcel[] =
                    '$1' . trim($excelBoolean) . '$2';
            }
        }
    ...
    public static function wrapResult($value)
    {
        if (is_string($value)) {
            ...
            if (
                preg_match(
                    '/^' . self::CALCULATION_REGEXP_ERROR . '$/i',
                    $value,
                    $match
                )
            ) {
    ...
    public function calculateCellValue(
        ...
        try {
            $result = self::unwrapResult(
                $this->_calculateFormulaValue(
                    $pCell->getValue(),
                    $pCell->getCoordinate(),
                    $pCell
                )
        ...
        if ($result === null) {
            ...
            is_float($result) &&
            (is_nan($result) || is_infinite($result))
        ) {
            return PHPExcel_Calculation_Functions::NaN();
        }
        return $result;
    ...
    public function calculateFormula(
        ...
        try {
            $result = self::unwrapResult(
                $this->_calculateFormulaValue($formula, $cellID, $pCell)
            );
        } catch (PHPExcel_Exception $e) {
            throw new PHPExcel_Calculation_Exception($e->getMessage());
        }
    ...
    public function getValueFromCache($cellReference, &$cellValue)
    {
        ...
        $this->_debugLog->writeDebugLog(
            'Testing cache value for cell ',
            $cellReference
        );
        if (
     ...
     * @throws    PHPExcel_Calculation_Exception
     */
    public function _calculateFormulaValue(
        $formula,
        $cellID = null,
        PHPExcel_Cell $pCell = null
    ) {
        ...
        $cellValue = $this->processTokenStack(
            $this->_parseFormula($formula, $pCell),
            $cellID,
            $pCell
        );
     ...
     *                                            2 = extend to fit
     */
    private static function checkMatrixOperands(
        &$operand1,
        &$operand2,
        $resize = 1
    ) {
        ...
        if (!is_array($operand1)) {
            list($matrixRows, $matrixColumns) = self::getMatrixDimensions(
                $operand2
            );
            $operand1 = array_fill(
                0,
                $matrixRows,
                array_fill(0, $matrixColumns, $operand1)
            );
            $resize = 0;
            ...
            $operand2 = array_fill(
                0,
                $matrixRows,
                array_fill(0, $matrixColumns, $operand2)
            );
            $resize = 0;
        }

        list($matrix1Rows, $matrix1Columns) = self::getMatrixDimensions(
            $operand1
        );
        list($matrix2Rows, $matrix2Columns) = self::getMatrixDimensions(
            $operand2
        );
        if (
            $matrix1Rows == $matrix2Columns &&
     ...
     */
    private static function resizeMatricesShrink(
        &$matrix1,
        &$matrix2,
        $matrix1Rows,
        $matrix1Columns,
     ...
     * @param    integer        $matrix2Columns    Column size of second matrix operand
     */
    private static function resizeMatricesExtend(
        &$matrix1,
        &$matrix2,
        $matrix1Rows,
        $matrix1Columns,
        ...
        if ($this->_debugLog->getWriteDebugLog()) {
            ...
            if (is_array($value)) {
                ...
                foreach ($value as $row) {
                    if (is_array($row)) {
                        $returnMatrix[] = implode(
                            $pad,
                            array_map([$this, 'showValue'], $row)
                        );
                        $rpad = '; ';
                    } else {
    ...
    private function convertMatrixReferences($formula)
    {
        ...
        if (strpos($formula, '{') !== false) {
            ...
            if ($openCount < $closeCount) {
                if ($openCount > 0) {
                    return $this->raiseFormulaError(
                        "Formula Error: Mismatched matrix braces '}'"
                    );
                } else {
                    return $this->raiseFormulaError(
                        "Formula Error: Unexpected '}' encountered"
                    );
                }
    ...
    {
        if (
            ($formula = $this->convertMatrixReferences(trim($formula))) ===
            false
        ) {
            return false;
        }
        ...
        while (true) {
            ...
            $isOperandOrFunction = preg_match(
                $regexpMatchString,
                substr($formula, $index),
                $match
            );
            ...
            if ($opCharacter == '-' && !$expectingOperator) {
                ...
                (isset(self::$operators[$opCharacter]) or
                    $isOperandOrFunction) &&
                $expectingOperator
            ) {
                ...
                if (
                    preg_match(
                        '/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i',
                        $d['value'],
                        $matches
                    )
                ) {
                    ...
                    if (isset(self::$controlFunctions[$functionName])) {
                        ...
                        $expectedArgumentCount =
                            self::$controlFunctions[$functionName][
                                'argumentCount'
                            ];
                        $functionCall =
                            self::$controlFunctions[$functionName][
                        ...
                        $expectedArgumentCount =
                            self::$PHPExcelFunctions[$functionName][
                                'argumentCount'
                            ];
                        $functionCall =
                            self::$PHPExcelFunctions[$functionName][
                    ...
                    if ($argumentCountError) {
                        return $this->raiseFormulaError(
                            "Formula Error: Wrong number of arguments for $functionName() function: $argumentCount given, " .
                                $expectedArgumentCountString .
                                ' expected'
                        );
                    }
                ...
                if ($expectingOperand || !$expectingOperator) {
                    $output[] = [
                        'type' => 'NULL Value',
                        'value' => self::$excelConstants['NULL'],
                        'reference' => null,
                    ];
                }
                ...
                if (
                    ...
                    if (
                        isset(
                            self::$PHPExcelFunctions[strtoupper($matches[1])]
                        ) ||
                        isset(self::$controlFunctions[strtoupper($matches[1])])
                    ) {
                        // it's a function
                        $stack->push('Function', strtoupper($val));
                        $ax = preg_match(
                            '/^\s*(\s*\))/ui',
                            substr($formula, $index + $length),
                            $amatch
                        );
                        if ($ax) {
                            $stack->push(
                                'Operand Count for Function ' .
                                    strtoupper($val) .
                                    ')',
                                0
                            );
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if ($matches[2] == '') {
                            ...
                            return $this->raiseFormulaError(
                                '3D Range references are not yet supported'
                            );
                        }
                    }
                    ...
                    if ($testPrevOp['value'] == ':') {
                        ...
                        if (strpos('!', $startRowColRef) !== false) {
                            list($rangeWS1, $startRowColRef) = explode(
                                '!',
                                $startRowColRef
                            );
                        }
                        ...
                        if (
                            ...
                            $endRowColRef =
                                $pCellParent !== null
                                    ? $pCellParent->getHighestColumn()
                                    : 'XFD'; //    Max 16,384 columns for Excel2007
                            $output[count($output) - 1]['value'] =
                                $rangeWS1 . 'A' . $startRowColRef;
                            $val = $rangeWS2 . $endRowColRef . $val;
                        } elseif (
                            ctype_alpha($startRowColRef) &&
                            ctype_alpha($val) &&
                            ...
                            $endRowColRef =
                                $pCellParent !== null
                                    ...
                                    : 1048576; //    Max 1,048,576 rows for Excel2007
                            $output[count($output) - 1]['value'] =
                    ...
                    if ($opCharacter == '"') {
                        ...
                        $val = self::wrapResult(
                            str_replace('""', '"', self::unwrapResult($val))
                        );
                ...
                isset(self::$operators[$opCharacter]) &&
                !$expectingOperator
            ) {
                return $this->raiseFormulaError(
            ...
            if ($formula[$index] == ' ') {
                ...
                if (
                    $expectingOperator &&
                    preg_match(
                        '/^' . self::CALCULATION_REGEXP_CELLREF . '.*/Ui',
                        substr($formula, $index),
                        $match
                    ) &&
    ...
    private function processTokenStack(
        $tokens,
        $cellID = null,
        PHPExcel_Cell $pCell = null
    ) {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if (isset(self::$binaryOperators[$token])) {
                ...
                if ($token == ':') {
                    $this->_debugLog->writeDebugLog(
                        'Evaluating Range ',
                        $this->showValue($operand1Data['reference']),
                        ' ',
                        $token,
                        ' ',
                ...
                switch ($token) {
                    ...
                    case '=': //    Equality
                    case '<>': //    Inequality
                        $this->executeBinaryComparisonOperation(
                            $cellID,
                            $operand1,
                            $operand2,
                            $token,
                    ...
                    case ':': //    Range
                        ...
                        if (strpos($operand1Data['reference'], '!') !== false) {
                            list($sheet1, $operand1Data['reference']) = explode(
                                '!',
                        ...
                        if (strpos($operand2Data['reference'], '!') !== false) {
                            list($sheet2, $operand2Data['reference']) = explode(
                                '!',
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            $oData = array_merge(
                                explode(':', $operand1Data['reference']),
                                explode(':', $operand2Data['reference'])
                            );
                            $oCol = $oRow = [];
                            foreach ($oData as $oDatum) {
                                $oCR = PHPExcel_Cell::coordinateFromString(
                                    $oDatum
                                );
                                $oCol[] =
                            ...
                            $cellRef =
                                ...
                                max($oRow);
                            if ($pCellParent !== null) {
                                $cellValue = $this->extractCellRange(
                                    $cellRef,
                                    $this->workbook->getSheetByName($sheet1),
                                    false
                                );
                            } else {
                                return $this->raiseFormulaError(
                                    'Unable to access Cell Reference'
                                );
                            }
                            $stack->push(
                                'Cell Reference',
                            ...
                            $stack->push(
                                'Error',
                                PHPExcel_Calculation_Functions::REF(),
                                null
                            );
                        }
                        break;
                    case '+': //    Addition
                        $this->executeNumericBinaryOperation(
                            $cellID,
                            $operand1,
                            $operand2,
                            $token,
                    ...
                    case '&': //    Concatenation
                        ...
                        if (is_array($operand1) || is_array($operand2)) {
                            ...
                            try {
                                ...
                                $matrix = new PHPExcel_Shared_JAMA_Matrix(
                                    $operand1
                                );
                            ...
                            $result =
                                ...
                                str_replace(
                                    '""',
                                    '"',
                                    self::unwrapResult($operand1, '"') .
                                        self::unwrapResult($operand2, '"')
                                ) .
                                '"';
                        }
                        $this->_debugLog->writeDebugLog(
                        ...
                        break;
                    case '|': //    Intersect
                        $rowIntersect = array_intersect_key(
                            $operand1,
                            $operand2
                        );
                        ...
                        foreach (array_keys($rowIntersect) as $row) {
                            ...
                            foreach ($rowIntersect[$row] as $col => $data) {
                                $oCol[] =
                                    PHPExcel_Cell::columnIndexFromString($col) -
                                    1;
                                $cellIntersect[$row] = array_intersect_key(
                                    $operand1[$row],
                                    $operand2[$row]
                                );
                            }
                        ...
                        $cellRef =
                            PHPExcel_Cell::stringFromColumnIndex(min($oCol)) .
                            min($oRow) .
                            ':' .
                            PHPExcel_Cell::stringFromColumnIndex(max($oCol)) .
                            max($oRow);
                        $this->_debugLog->writeDebugLog(
                            'Evaluation Result is ',
                ...
                if (isset($matches[8])) {
                    ...
                    if ($pCell === null) {
                        ...
                        if ($matches[2] > '') {
                            ...
                            if ($pCellParent !== null) {
                                $cellSheet = $this->workbook->getSheetByName(
                                    $matches[2]
                                );
                                if (
                                    $cellSheet &&
                ...
                if ($functionName != 'MKMATRIX') {
                    $this->_debugLog->writeDebugLog(
                        ...
                        '() with ',
                        $argCount == 0 ? 'no' : $argCount,
                        ' argument',
                        $argCount == 1 ? '' : 's'
                    );
                }
                if (
                    isset(self::$PHPExcelFunctions[$functionName]) ||
                    ...
                    if (isset(self::$PHPExcelFunctions[$functionName])) {
                        ...
                        $passByReference = isset(
                            self::$PHPExcelFunctions[$functionName][
                                'passByReference'
                            ]
                        );
                        $passCellReference = isset(
                            self::$PHPExcelFunctions[$functionName][
                                'passCellReference'
                            ]
                        );
                    ...
                    if (strpos($functionCall, '::') !== false) {
                        $result = call_user_func_array(
                            explode('::', $functionCall),
                            $args
                        );
                    } else {
                        foreach ($args as &$arg) {
                            $arg = PHPExcel_Calculation_Functions::flattenSingleValue(
                                $arg
                            );
                        }
                        unset($arg);
                ...
                if (isset(self::$excelConstants[strtoupper($token)])) {
                    ...
                    preg_match(
                        '/^' . self::CALCULATION_REGEXP_NAMEDRANGE . '$/i',
                        $token,
                        $matches
                    )
                ) {
                    ...
                    $this->_debugLog->writeDebugLog(
                        'Evaluating Named Range ',
                        $namedRange
                    );
                    $cellValue = $this->extractNamedRange(
                        $namedRange,
                        null !== $pCell ? $pCellWorksheet : null,
                        false
                    );
                    $pCell->attach($pCellParent);
                    $this->_debugLog->writeDebugLog(
                        'Evaluation Result for named range ',
                        $namedRange,
                        ' is ',
                        $this->showTypeDetails($cellValue)
                    );
        ...
        if (is_string($operand)) {
            ...
            if (!is_numeric($operand)) {
                ...
                if ($operand > '' && $operand[0] == '#') {
                    ...
                    $this->_debugLog->writeDebugLog(
                        'Evaluation Result is ',
                        $this->showTypeDetails($operand)
                    );
                    return false;
                } elseif (
                    !PHPExcel_Shared_String::convertToNumberIfFraction($operand)
                ) {
                    //    If not a numeric or a fraction, then it's a text string, and so can't be used in mathematical binary operations
                    $stack->push('Value', '#VALUE!');
                    $this->_debugLog->writeDebugLog(
                        'Evaluation Result is a ',
                        $this->showTypeDetails('#VALUE!')
                    );
                    return false;
                }
            }
    ...
    private function executeBinaryComparisonOperation(
        $cellID,
        $operand1,
        $operand2,
        $operation,
        ...
        if (is_array($operand1) || is_array($operand2)) {
            ...
            $this->_debugLog->writeDebugLog(
                'Comparison Evaluation Result is ',
                $this->showTypeDetails($result)
            );
        ...
        switch ($operation) {
            ...
            case '>=':
                if (is_numeric($operand1) && is_numeric($operand2)) {
                    $result =
                        abs($operand1 - $operand2) < $this->delta ||
                        $operand1 > $operand2;
    ...
    private function executeNumericBinaryOperation(
        $cellID,
        $operand1,
        $operand2,
        $operation,
     ...
     * @throws    PHPExcel_Calculation_Exception
     */
    public function extractCellRange(
        &$pRange = 'A1',
        ...
        if ($pSheet !== null) {
            ...
            if (strpos($pRange, '!') !== false) {
                ...
                list(
                    $pSheetName,
                    $pRange,
                ) = PHPExcel_Worksheet::extractSheetTitle($pRange, true);
            ...
            // Extract range
            $aReferences = PHPExcel_Cell::extractAllCellReferencesInRange(
                $pRange
            );
     ...
     * @throws    PHPExcel_Calculation_Exception
     */
    public function extractNamedRange(
        &$pRange = 'A1',
        PHPExcel_Worksheet $pSheet = null,
        $resetLog = true
    ) {
        ...
        if ($pSheet !== null) {
            ...
            if ($namedRange !== null) {
                ...
                if (ctype_alpha($splitRange[0][0])) {
                    $pRange =
                        ...
                        '1:' .
                        $splitRange[0][1] .
                        $namedRange->getWorksheet()->getHighestRow();
            ...
            if (!isset($aReferences[1])) {
                ...
                list(
                    $currentCol,
                    $currentRow,
                ) = PHPExcel_Cell::coordinateFromString($aReferences[0]);
                $cellValue = null;
                if ($pSheet->cellExists($aReferences[0])) {
                    $returnValue[$currentRow][$currentCol] = $pSheet
                        ->getCell($aReferences[0])
                ...
                foreach ($aReferences as $reference) {
                    ...
                    list(
                        $currentCol,
                        $currentRow,
                    ) = PHPExcel_Cell::coordinateFromString($reference);
    ...
    public function isImplemented($pFunction = '')
    {
        ...
        if (isset(self::$PHPExcelFunctions[$pFunction])) {
            return self::$PHPExcelFunctions[$pFunction]['functionCall'] !=

...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function close()
    {
        ...
        // Column dimensions
        if (
            ($defaultWidth = $phpSheet
                ->getDefaultColumnDimension()
                ->getWidth()) < 0
        ) {
            $defaultWidth = PHPExcel_Shared_Font::getDefaultColumnWidthByFont(
                $phpSheet
                    ->getParent()
                    ->getDefaultStyle()
                    ->getFont()
        ...
        foreach ($phpSheet->getRowDimensions() as $rowDimension) {
            ...
            $this->writeRow(
                ...
                $rowDimension->getRowHeight(),
                $xfIndex,
                $rowDimension->getVisible() ? '0' : '1',
        ...
        foreach ($phpSheet->getCellCollection() as $cellID) {
            ...
            $row = $cell->getRow() - 1;
            $column =
                PHPExcel_Cell::columnIndexFromString($cell->getColumn()) - 1;
            ...
            if ($cVal instanceof PHPExcel_RichText) {
                ...
                $arrcRun = [];
                $str_len = PHPExcel_Shared_String::CountCharacters(
                    $cVal->getPlainText(),
                    'UTF-8'
                );
                $str_pos = 0;
                ...
                foreach ($elements as $element) {
                    ...
                    // Position FROM
                    $str_pos += PHPExcel_Shared_String::CountCharacters(
                        $element->getText(),
                        'UTF-8'
                    );
                }
                $this->writeRichTextString(
                    $row,
                    $column,
                    $cVal->getPlainText(),
                    $xfIndex,
        ...
        // Hyperlinks
        foreach (
            $phpSheet->getHyperLinkCollection()
            as $coordinate => $hyperlink
        ) {
            list($column, $row) = PHPExcel_Cell::coordinateFromString(
                $coordinate
            );
            ...
            $this->writeUrl(
                $row - 1,
                PHPExcel_Cell::columnIndexFromString($column) - 1,
                $url
            );
        }
        ...
        if (!empty($arrConditionalStyles)) {
            ...
            foreach (
                ...
                as $cellCoordinate => $conditionalStyles
            ) {
                foreach ($conditionalStyles as $conditional) {
                    if (
                        $conditional->getConditionType() ==
                            PHPExcel_Style_Conditional::CONDITION_EXPRESSION ||
                        $conditional->getConditionType() ==
                            PHPExcel_Style_Conditional::CONDITION_CELLIS
                    ) {
                        if (
                            !in_array(
    ...
    private function writeLabel($row, $col, $str, $xfIndex)
    {
        ...
        if ($strlen > $this->xlsStringMaxLength) {
            ...
            if (is_bool($calculatedValue)) {
                ...
                if (
                    ...
                    $num = pack(
                        ...
                        0x02,
                        0x00,
                        self::mapErrorCode($calculatedValue),
                        0x00,
                        0x00,
                        0xffff
                    );
    ...
    public function writeUrlWeb($row1, $col1, $row2, $col2, $url)
    {
        ...
        $this->append(
            $header . $data . $unknown1 . $options . $unknown2 . $url_len . $url
        );
        return 0;
    }
    ...
    public function writeUrlInternal($row1, $col1, $row2, $col2, $url)
    {
        ...
        $url = PHPExcel_Shared_String::ConvertEncoding(
            $url,
            'UTF-16LE',
            'UTF-8'
        );
    ...
    private function writeWindow2()
    {
        ...
        $fSelected =
            $this->phpSheet === $this->phpSheet->getParent()->getActiveSheet()
                ? 1
                : 0;
        $fPaged = 1; // 2
        $fPageBreakPreview =
            $this->phpSheet->getSheetView()->getView() ===
            PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
        ...
        $zoom_factor_page_break = $fPageBreakPreview
            ? $this->phpSheet->getSheetView()->getZoomScale()
            : 0x0000;
        $zoom_factor_normal = $this->phpSheet
            ->getSheetView()
            ->getZoomScaleNormal();
    ...
    {
        $defaultRowHeight = $this->phpSheet
    ...
    private function writeSelection()
    {
        ...
        $selectedCells = PHPExcel_Cell::splitRange(
            $this->phpSheet->getSelectedCells()
        );
    ...
    private function writeMergedCells()
    {
        ...
        foreach ($mergeCells as $mergeCell) {
            ...
            list($firstColumn, $firstRow) = PHPExcel_Cell::coordinateFromString(
                $first
            );
            list($lastColumn, $lastRow) = PHPExcel_Cell::coordinateFromString(
                $last
            );
            ...
            $recordData .= pack(
                ...
                $firstRow - 1,
                $lastRow - 1,
                PHPExcel_Cell::columnIndexFromString($firstColumn) - 1,
                PHPExcel_Cell::columnIndexFromString($lastColumn) - 1
            );
    ...
    private function writeSheetProtection()
    {
        ...
        // prepare options
        $options =
            (int) !$this->phpSheet->getProtection()->getObjects() |
            ...
            ((int) !$this->phpSheet->getProtection()->getInsertHyperlinks() <<
                7) |
            ((int) !$this->phpSheet->getProtection()->getDeleteColumns() << 8) |
            ((int) !$this->phpSheet->getProtection()->getDeleteRows() << 9) |
            ((int) !$this->phpSheet->getProtection()->getSelectLockedCells() <<
                10) |
    ...
    private function writeRangeProtection()
    {
        foreach ($this->phpSheet->getProtectedCells() as $range => $password) {
            ...
            foreach ($cellRanges as $cellRange) {
                $recordData .= $this->writeBIFF8CellRangeAddressFixed(
                    $cellRange
                );
            }
            ...
            $recordData .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                'p' . md5($recordData)
            );
    ...
    private function writeSetup()
    {
        ...
        $iScale = $this->phpSheet->getPageSetup()->getScale()
            ? $this->phpSheet->getPageSetup()->getScale()
            : 100; // Print scaling factor
    ...
    private function writeHcenter()
    {
        ...
        $fHCenter = $this->phpSheet->getPageSetup()->getHorizontalCentered()
            ? 1
            : 0; // Horizontal centering
    ...
    private function writeVcenter()
    {
        ...
        $fVCenter = $this->phpSheet->getPageSetup()->getVerticalCentered()
            ? 1
            : 0; // Horizontal centering
    ...
    private function writeAutoFilterInfo()
    {
        ...
        $rangeBounds = PHPExcel_Cell::rangeBoundaries(
            $this->phpSheet->getAutoFilter()->getRange()
        );
        ...
        foreach ($this->phpSheet->getBreaks() as $cell => $breakType) {
            ...
            switch ($breakType) {
                case PHPExcel_Worksheet::BREAK_COLUMN:
                    ...
                    $vbreaks[] =
                        PHPExcel_Cell::columnIndexFromString($coordinates[0]) -
                        1;
                    break;
    ...
    private function writePassword()
    {
        ...
        if (
            !$this->phpSheet->getProtection()->getSheet() ||
    ...
    public function positionImage(
        ...
        if (
            ...
            PHPExcel_Shared_Excel5::sizeCol(
                $this->phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_start)
            )
        ) {
            $x1 = 0;
        }
        if (
            $y1 >=
            PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_start + 1)
        ) {
            $y1 = 0;
        }
        ...
        while (
            ...
            PHPExcel_Shared_Excel5::sizeCol(
                $this->phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_end)
            )
        ) {
            $width -= PHPExcel_Shared_Excel5::sizeCol(
                $this->phpSheet,
                PHPExcel_Cell::stringFromColumnIndex($col_end)
            );
            ++$col_end;
        ...
        while (
            $height >=
            PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1)
        ) {
            $height -= PHPExcel_Shared_Excel5::sizeRow(
                $this->phpSheet,
                $row_end + 1
            );
            ++$row_end;
        ...
        if (
            PHPExcel_Shared_Excel5::sizeRow($this->phpSheet, $row_end + 1) == 0
        ) {
            return;
        }
        ...
        $x2 =
            ...
            1024; // Distance to right side of object
        $y2 =
            ($height /
                PHPExcel_Shared_Excel5::sizeRow(
                    $this->phpSheet,
     ...
     */
    private function writeObjPicture(
        $colL,
        $dxL,
        $rwT,
        $dyT,
        ...
        for ($j = $height; $j--; ) {
            for ($i = 0; $i < $width; ++$i) {
                ...
                foreach (['red', 'green', 'blue'] as $key) {
                    $color[$key] =
                        $color[$key] +
                        round(((255 - $color[$key]) * $color['alpha']) / 127);
                }
                $data .=
                    chr($color['blue']) .
                    chr($color['green']) .
    ...
    private function writeDataValidity()
    {
        ...
        if (!empty($dataValidationCollection)) {
            ...
            foreach (
                ...
                as $cellCoordinate => $dataValidation
            ) {
                // initialize record data
                $data = '';
                ...
                if (
                    $type == 0x03 &&
                    preg_match('/^\".*\"$/', $dataValidation->getFormula1())
                ) {
                    $options |= 0x01 << 7;
                }
                ...
                $promptTitle =
                    $dataValidation->getPromptTitle() !== ''
                        ? $dataValidation->getPromptTitle()
                        : chr(0);
                $data .= PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                    $promptTitle
                );
                ...
                $data .= pack('v', 0x0001);
                $data .= $this->writeBIFF8CellRangeAddressFixed(
                    $cellCoordinate
                );
    ...
    private function writePageLayoutView()
    {
        ...
        if (
            $this->phpSheet->getSheetView()->getView() ==
            PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT
        ) {
            $fPageLayoutView = 1;
        } else {
            $fPageLayoutView = 0;
    ...
    private function writeCFRule(PHPExcel_Style_Conditional $conditional)
    {
        ...
        if ($numConditions == 1) {
            ...
            $conditional->getOperatorType() ==
                PHPExcel_Style_Conditional::OPERATOR_BETWEEN
        ) {
            $szValue1 = $arrConditions[0] <= 65535 ? 3 : 0x0000;
        ...
        $bBorderLeft =
            $conditional
                ...
                ->getLeft()
                ->getColor()
                ->getARGB() == PHPExcel_Style_Color::COLOR_BLACK &&
            $conditional
                ->getStyle()
                ->getBorders()
                ->getLeft()
        ...
        if ($bFormatFont == 1) {
            ...
            if (
                ...
                $dataBlockFont = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
                    $conditional
                        ->getStyle()
                        ->getFont()
                        ->getName()
    ...
    private function writeCFHeader()
    {
        ...
        $arrConditional = [];
        foreach (
            $this->phpSheet->getConditionalStylesCollection()
            as $cellCoordinate => $conditionalStyles
        ) {
            foreach ($conditionalStyles as $conditional) {
                if (
                    ...
                    if (
                        !in_array($conditional->getHashCode(), $arrConditional)
                    ) {
                        $arrConditional[] = $conditional->getHashCode();
                    }
                    // Cells
                    $arrCoord = PHPExcel_Cell::coordinateFromString(
                        $cellCoordinate
                    );
                    if (!is_numeric($arrCoord[0])) {
                        $arrCoord[0] = PHPExcel_Cell::columnIndexFromString(
                            $arrCoord[0]
                        );
                    }
                    if (
                        is_null($numColumnMin) ||
                        $numColumnMin > $arrCoord[0]
                    ) {
                        $numColumnMin = $arrCoord[0];
                    }
                    if (
                        is_null($numColumnMax) ||

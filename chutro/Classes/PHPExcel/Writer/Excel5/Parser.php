...
class PHPExcel_Writer_Excel5_Parser
{
    ...
    private function convert($token)
    {
        if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $token)) {
            ...
            preg_match('/^#[A-Z0\/]{3,5}[!?]{1}$/', $token) or
            $token == '#N/A'
        ) {
    ...
    private function convertString($string)
    {
        ...
        return pack('C', $this->ptg['ptgStr']) .
            PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($string);
    }
    ...
    private function convertRange3d($token)
    {
        ...
        if (preg_match("/^(\\$)?[A-Ia-i]?[A-Za-z](\\$)?(\d+)$/", $cell1)) {
            ...
            list($row1, $col1, $row2, $col2) = $this->rangeToPackedRange(
                $cell1 . ':' . $cell2
            );
        }
    ...
    private function cellToPackedRowcol($cell)
    {
        ...
        if ($row >= 65536) {
            throw new PHPExcel_Writer_Exception(
                "Row in: $cell greater than 65536 "
            );
        }
    ...
    private function rangeToPackedRange($range)
    {
        ...
        if ($row1 >= 65536 or $row2 >= 65536) {
            throw new PHPExcel_Writer_Exception(
                "Row in: $range greater than 65536 "
            );
        }
    ...
    private function match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                if (
                    preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $token) and
                    !preg_match('/[0-9]/', $this->lookAhead) and
                    $this->lookAhead != ':' and
                    $this->lookAhead != '.' and
                    $this->lookAhead != '!'
                ) {
                    ...
                    preg_match(
                        '/^' .
                            ...
                            '(\:' .
                            self::REGEX_SHEET_TITLE_UNQUOTED .
                            ")?\!\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+:\\$?([A-Ia-i]?[A-Za-z])?\\$?[0-9]+$/u",
                        $token
                    ) and !preg_match('/[0-9]/', $this->lookAhead)
                ) {
                    ...
                    is_numeric($token) and
                    (!is_numeric($token . $this->lookAhead) or
                        $this->lookAhead == '') and
                    $this->lookAhead != '!' and
                    $this->lookAhead != ':'
                ) {
    ...
    private function expression()
    {
        ...
        if (preg_match("/\"([^\"]|\"\"){0,255}\"/", $this->currentToken)) {
            ...
            preg_match('/^#[A-Z0\/]{3,5}[!?]{1}$/', $this->currentToken) or
    ...
    private function fact()
    {
        ...
        // if it's a reference
        if (
            preg_match('/^\$?[A-Ia-i]?[A-Za-z]\$?[0-9]+$/', $this->currentToken)
        ) {
            ...
            preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/i", $this->currentToken)
        ) {
            // if it's a function call
            $result = $this->func();
            return $result;
    ...
    public function toReversePolish($tree = [])
    {
        ...
        if (
            preg_match("/^[A-Z0-9\xc0-\xdc\.]+$/", $tree['value']) and
            ...
            return $left_tree .
                $this->convertFunction($tree['value'], $tree['right']);
        } else {

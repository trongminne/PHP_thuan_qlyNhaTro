...
class PHPExcel_Writer_Excel5_Workbook extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    public function addFont(PHPExcel_Style_Font $font)
    {
        ...
        if (isset($this->addedFonts[$fontHashCode])) {
            ...
            $fontWriter->setColorIndex(
                $this->addColor($font->getColor()->getRGB())
            );
    ...
    public function writeWorkbook($pWorksheetSizes = null)
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            $this->writeBoundSheet(
                $this->phpExcel->getSheet($i),
    ...
    private function writeAllNumberFormats()
    {
        foreach ($this->numberFormats as $numberFormatIndex => $numberFormat) {
            $this->writeNumberFormat(
                $numberFormat->getFormatCode(),
                $numberFormatIndex
            );
        }
    }
    ...
    private function writeExternals()
    {
        ...
        for ($i = 0; $i < $countSheets; ++$i) {
            $this->writeExternalSheet(
                $this->phpExcel->getSheet($i)->getTitle()
            );
        }
    }
    ...
    private function writeNames()
    {
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                // Print area
                $printArea = PHPExcel_Cell::splitRange(
                    $sheetSetup->getPrintArea()
                );
                $printArea = $printArea[0];
                $printArea[0] = PHPExcel_Cell::coordinateFromString(
                    $printArea[0]
                );
                $printArea[1] = PHPExcel_Cell::coordinateFromString(
                    $printArea[1]
                );
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if (
                $sheetSetup->isColumnsToRepeatAtLeftSet() &&
                ...
                if ($sheetSetup->isColumnsToRepeatAtLeftSet()) {
                    $repeat = $sheetSetup->getColumnsToRepeatAtLeft();
                    $colmin =
                        PHPExcel_Cell::columnIndexFromString($repeat[0]) - 1;
                    $colmax =
                        PHPExcel_Cell::columnIndexFromString($repeat[1]) - 1;
                } else {
                    $colmin = 0;
                    $colmax = 255;
                }
        ...
        if (count($this->phpExcel->getNamedRanges()) > 0) {
            ...
            foreach ($namedRanges as $namedRange) {
                ...
                for ($i = 0; $i < count($range); $i++) {
                    ...
                    if (isset($range[$i][1])) {
                        $range[$i][1] = PHPExcel_Cell::absoluteCoordinate(
                            $range[$i][1]
                        );
                    }
                }
                ...
                try {
                    ...
                    if (
                        isset($formulaData[0]) and
                        ($formulaData[0] == "\x7A" or $formulaData[0] == "\x5A")
                    ) {
                    ...
                    if ($namedRange->getLocalOnly()) {
                        // local scope
                        $scope =
                            $this->phpExcel->getIndex($namedRange->getScope()) +
                            1;
                    } else {
                        // global scope
                        $scope = 0;
                    }
                    $chunk .= $this->writeData(
                        $this->writeDefinedNameBiff8(
                            $namedRange->getName(),
                            $formulaData,
                            $scope,
                            false
        ...
        for ($i = 0; $i < $total_worksheets; ++$i) {
            ...
            if ($sheetSetup->isPrintAreaSet()) {
                ...
                for ($j = 0; $j < $countPrintArea; ++$j) {
                    ...
                    $printAreaRect[0] = PHPExcel_Cell::coordinateFromString(
                        $printAreaRect[0]
                    );
                    $printAreaRect[1] = PHPExcel_Cell::coordinateFromString(
                        $printAreaRect[1]
                    );
     ...
     */
    private function writeDefinedNameBiff8(
        $name,
        $formulaData,
        $sheetIndex = 0,
        $isBuiltIn = false
        ...
        $name = substr(
            PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($name),
            2
        );
     ...
     * @return    string    Complete binary record data
     * */
    private function writeShortNameBiff8(
        $name,
        $sheetIndex = 0,
        $rangeBounds,
        $isHidden = false
    ...
    private function writeNumberFormat($format, $ifmt)
    {
        ...
        $numberFormatString = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong(
            $format
        );
     ...
     */
    private function writeNameShort(
        $index,
        $type,
        $rowmin,
        $rowmax,
     ...
     * @param integer $colmax End column
     */
    private function writeNameLong(
        $index,
        $type,
        $rowmin,
        $rowmax,
        ...
        foreach (array_keys($this->stringTable) as $string) {
            ...
            while ($finished === false) {
                ...
                if (strlen($recordData) + strlen($string) <= $continue_limit) {
                    ...
                    if (
                        strlen($recordData) + strlen($string) ==
                        $continue_limit
                    ) {
                    ...
                    if ($space_remaining < $min_space_needed) {
                        ...
                        if (
                            $encoding == 1 &&
                            (strlen($string) - $space_remaining) % 2 == 1
                        ) {

...
class PHPExcel_ReferenceHelper
{
    ...
    private static function cellAddressInDeleteRange(
        ...
        list($cellColumn, $cellRow) = PHPExcel_Cell::coordinateFromString(
            $cellAddress
        );
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustPageBreaks(
        ...
        foreach ($aBreaks as $key => $value) {
            if (
                ...
                $newReference = $this->updateCellReference(
                    $key,
                    $pBefore,
                    $pNumCols,
                    $pNumRows
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustComments(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustHyperlinks(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustDataValidations(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
        ...
        $pNumCols > 0 || $pNumRows > 0
            ? uksort($aDataValidationCollection, [
                'PHPExcel_ReferenceHelper',
                'cellReverseSort',
            ])
            : uksort($aDataValidationCollection, [
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustMergeCells(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustProtectedCells(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustColumnDimensions(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
        ...
        $aColumnDimensions = array_reverse(
            $pSheet->getColumnDimensions(),
            true
        );
        if (!empty($aColumnDimensions)) {
            foreach ($aColumnDimensions as $objColumnDimension) {
                $newReference = $this->updateCellReference(
                    $objColumnDimension->getColumnIndex() . '1',
                    $pBefore,
                    $pNumCols,
                    $pNumRows
                );
                list($newReference) = PHPExcel_Cell::coordinateFromString(
                    $newReference
                );
     ...
     * @param   integer             $pNumRows           Number of rows to insert/delete (negative values indicate deletion)
     */
    protected function adjustRowDimensions(
        $pSheet,
        $pBefore,
        $beforeColumnIndex,
        $pNumCols,
        ...
        if (!empty($aRowDimensions)) {
            ...
            for ($i = $beforeRow; $i <= $beforeRow - 1 + $pNumRows; ++$i) {
                ...
                $newDimension->setOutlineLevel(
                    $copyDimension->getOutlineLevel()
                );
    ...
    public function insertNewBefore(
        ...
        $beforeColumn = 'A';
        $beforeRow = 1;
        list($beforeColumn, $beforeRow) = PHPExcel_Cell::coordinateFromString(
            $pBefore
        );
        $beforeColumnIndex = PHPExcel_Cell::columnIndexFromString(
            $beforeColumn
        );
        ...
        if ($pNumRows < 0 && $beforeRow - 1 + $pNumRows > 0) {
            for (
                ...
                $i <= PHPExcel_Cell::columnIndexFromString($highestColumn) - 1;
                ++$i
            ) {
        ...
        while ($cellID = array_pop($aCellCollection)) {
            ...
            $cellIndex = PHPExcel_Cell::columnIndexFromString(
                $cell->getColumn()
            );
            ...
            $newCoordinates =
                PHPExcel_Cell::stringFromColumnIndex(
                    $cellIndex - 1 + $pNumCols
                ) .
                ($cell->getRow() + $pNumRows);

            // Should the cell be updated? Move value and cellXf index from one cell to another.
            if (
                $cellIndex >= $beforeColumnIndex &&
                ...
                if (
                    $cell->getDataType() == PHPExcel_Cell_DataType::TYPE_FORMULA
                ) {
        ...
        if ($pNumCols > 0 && $beforeColumnIndex - 2 > 0) {
            for ($i = $beforeRow; $i <= $highestRow - 1; ++$i) {
                ...
                if ($pSheet->cellExists($coordinate)) {
                    ...
                    $conditionalStyles = $pSheet->conditionalStylesExists(
                        $coordinate
                    )
                    ...
                    for (
                        ...
                        $j <= $beforeColumnIndex - 2 + $pNumCols;
                        ++$j
                    ) {
                        $pSheet
                        ...
                        if ($conditionalStyles) {
                            ...
                            $pSheet->setConditionalStyles(
                                PHPExcel_Cell::stringFromColumnIndex($j) . $i,
                                $cloned
                            );
                        }
                    }
        ...
        if (!empty($autoFilterRange)) {
            if ($pNumCols != 0) {
                ...
                if (count($autoFilterColumns) > 0) {
                    ...
                    $columnIndex = PHPExcel_Cell::columnIndexFromString(
                        $column
                    );
                    list(
                        $rangeStart,
                        $rangeEnd,
                    ) = PHPExcel_Cell::rangeBoundaries($autoFilterRange);
                    if ($columnIndex <= $rangeEnd[0]) {
                        ...
                        if ($pNumCols > 0) {
                            ...
                            $startColID = PHPExcel_Cell::stringFromColumnIndex(
                                $startCol - 1
                            );
                            $toColID = PHPExcel_Cell::stringFromColumnIndex(
                                $startCol + $pNumCols - 1
                            );
                            $endColID = PHPExcel_Cell::stringFromColumnIndex(
                                $rangeEnd[0]
                            );
        ...
        if ($pSheet->getPageSetup()->isPrintAreaSet()) {
            $pSheet
                ...
                ->setPrintArea(
                    $this->updateCellReference(
                        $pSheet->getPageSetup()->getPrintArea(),
                        $pBefore,
                        $pNumCols,
                        $pNumRows
                    )
            ...
            if ($i = !$i) {
                    ...
                    foreach ($matches as $match) {
                        ...
                        if (
                            ...
                            if (
                                ...
                                $newCellTokens[$cellIndex] = preg_quote(
                                    $toString
                                );
                                $cellTokens[$cellIndex] =
                                    '/(?<!\d\$\!)' .
                                    preg_quote($fromString) .
                                    '(?!\d)/i';
                                ++$adjustCount;
                            }
                        }
                ...
                if ($matchCount > 0) {
                    foreach ($matches as $match) {
                        $fromString = $match[2] > '' ? $match[2] . '!' : '';
                        $fromString .= $match[3] . ':' . $match[4];
                        $modified3 = $this->updateCellReference(
                            $match[3],
                            $pBefore,
                            $pNumCols,
                            $pNumRows
                        );
                        $modified4 = $this->updateCellReference(
                            $match[4],
                            $pBefore,
                            $pNumCols,
                            $pNumRows
     ...
     * @throws    PHPExcel_Exception
     */
    public function updateCellReference(
        $pCellRange = 'A1',
        $pBefore = 'A1',
        $pNumCols = 0,
        $pNumRows = 0
        ...
        if (strpos($pCellRange, '!') !== false) {
            ...
            // Single cell
            return $this->updateSingleCellReference(
                $pCellRange,
                $pBefore,
                $pNumCols,
                $pNumRows
     ...
     */
    public function updateNamedFormulas(
        PHPExcel $pPhpExcel,
        $oldName = '',
        $newName = ''
    ) {
     ...
     * @throws    PHPExcel_Exception
     */
    private function updateCellRange(
        $pCellRange = 'A1:A1',
        $pBefore = 'A1',
        $pNumCols = 0,
        $pNumRows = 0
            ...
            for ($i = 0; $i < $ic; ++$i) {
                ...
                for ($j = 0; $j < $jc; ++$j) {
                    if (ctype_alpha($range[$i][$j])) {
                        $r = PHPExcel_Cell::coordinateFromString(
                            $this->updateSingleCellReference(
                                $range[$i][$j] . '1',
                                $pBefore,
                                $pNumCols,
                                $pNumRows
                        ...
                        $range[$i][$j] = $this->updateSingleCellReference(
                            $range[$i][$j],
                            $pBefore,
                            $pNumCols,
                            $pNumRows
     ...
     * @throws    PHPExcel_Exception
     */
    private function updateSingleCellReference(
        $pCellReference = 'A1',
        $pBefore = 'A1',
        $pNumCols = 0,
        $pNumRows = 0
        ...
        if (
            ...
            list(
                $beforeColumn,
                $beforeRow,
            ) = PHPExcel_Cell::coordinateFromString($pBefore);

            // Get coordinates of $pCellReference
            list($newColumn, $newRow) = PHPExcel_Cell::coordinateFromString(
                $pCellReference
            );
            ...
            $updateColumn =
                $newColumn[0] != '$' &&
                $beforeColumn[0] != '$' &&
                PHPExcel_Cell::columnIndexFromString($newColumn) >=
            ...
            if ($updateColumn) {
                $newColumn = PHPExcel_Cell::stringFromColumnIndex(
                    PHPExcel_Cell::columnIndexFromString($newColumn) -
                        1 +
                        $pNumCols
                );
